...
/**
 * QuickSort copy for working on PipelinedSorter.SortSpan instances.
 */
@InterfaceAudience.Private
@InterfaceStability.Unstable
public final class TezSortSpanQuickSort implements IndexedSorter {

  private static final IndexedSorter alt = new HeapSort();

  public TezSortSpanQuickSort() {
  }
  ...
  private static void sortInternal(final IndexedSortable sortable, int p, int r,
      final Progressable rep, int depth) {
    // this is where we can do a cast cheaply instead of dynamically
    // resolve actual type later in loop/hotspot (see: TEZ-3349)
    PipelinedSorter.SortSpan s = (PipelinedSorter.SortSpan) sortable;
    if (null != rep) {
      rep.progress();
    }
    while (true) {
